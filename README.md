...
**Fase 1 (MVP comercial) em andamento — Cadastros, PDV, Estoque básico, Financeiro básico, fiscal e pagamento integrado (sandbox) concluídos:**
...
- Frontend: `/painel/produtos` (cadastro + saldo de estoque + movimentação), `/painel/pdv` (abrir caixa, montar carrinho, pagamento dividido, sangria/suprimento, fechar caixa, confirmação de NFC-e sandbox) e `/painel/financeiro` (contas a pagar/receber + resumo de fluxo de caixa)
- 24 testes e2e contra Postgres real (Cadastros + PDV + Estoque + Financeiro + Fiscal/Pagamento) cobrindo CRUD, isolamento entre tenants (RLS), RBAC, regras de negócio do PDV, desconto atômico de estoque nas vendas, ciclo de vida de contas a pagar/receber, emissão fiscal (incluindo contingência/reemissão) e recusa de pagamento pelo gateway

Com isso os seis itens de Fase 1 do roadmap estão implementados. O que falta para produção real de fiscal/pagamento é uma decisão de negócio (qual parceiro, criar a conta) — não código; ver [docs/03](docs/03-build-vs-buy-pagamentos-fiscal.md).
