...
> **Nota (2026-08-18):** este é o documento de proposta original, anterior à Fase 0 — mantido como registro do raciocínio por trás das escolhas, não como status atual. Para o que foi de fato implementado, [README.pt-BR.md](../README.pt-BR.md) é a fonte de verdade (seção "Status atual", atualizada a cada módulo entregue; o [README.md](../README.md) em inglês tem um resumo mais enxuto). Alguns pontos abaixo divergiram da proposta durante a implementação real, porque a necessidade que os justificava nunca se materializou nesta escala — nenhum deles bloqueia nada hoje, mas vale saber antes de assumir que estão prontos:
